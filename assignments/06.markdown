...
| BAZ | 5837 |
```
		* Of course, using psql, you can simply copy and paste from the commandline into a markdown code black (fence with 3 backticks)
...
* Underneath a header `## Logical Model`, show the image in your `report.md` using markdown
...
⚠️ __Do 6 out of 9 of these in order__. Any additional queries will be counted as extra credit towards homework.
...
5. __Find only the date of most recently entered event(s)__ 📅
6. __Now display the report id, product name, and the date of the most recently entered event(s); there may be more than 1__ 📅📈
	* do not hardcode a limit
	* hint: a subquery may be useful for this
7. __Find the event that had the most symptoms (there could be a tie)__ 🤒
...
8. __What are the 3 most common symptom__ 🤮 
	* include the name of the symptom and the count 
	* sorted by the count from greatest to least
	* as secondary sort order, sort in alphabetical order of the name of the sumptom
9. __Find the incident reported that dealt with the youngest patient__ 👶
